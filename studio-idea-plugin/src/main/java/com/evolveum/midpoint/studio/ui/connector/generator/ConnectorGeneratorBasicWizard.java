...
public class ConnectorGeneratorBasicWizard extends AbstractWizard<Step> {
    ...
    private final List<ConnectorGeneratorWizardStep> myWizardStepsList = new ArrayList<>();
    ...
    private void buildSteps() {
        ...
        myWizardStepsList.add(new ApplicationIdentificationStep(this, client, dataModel, StepStateBadge.State.IN_PROGRESS));
        myWizardStepsList.add(new DiscoverDocumentationStep(this, client, dataModel, StepStateBadge.State.NONE));
        myWizardStepsList.add(new ConnectorIdentificationStep(this, client, dataModel, StepStateBadge.State.NONE));
        myWizardStepsList.add(new CreateConnectorStep(this, client, dataModel, StepStateBadge.State.NONE));
...
//    }
    ...
    public void updateNavigationMenuByLiveStates() {
       ...
       IntStream.range(0, myWizardStepsList.size()).forEach(
               i -> addStepIfStateNotNone(i, myWizardStepsList.get(i).getComponent().getName()));

        synchronizeMenuHighlight();
    }

    private void addStepIfStateNotNone(int stepIndex, String stepDisplayName) {
        ...
        if (step instanceof ConnectorGeneratorWizardStep generatorStep) {
            ...
            if (!currentState.equals(StepStateBadge.State.NONE)) {
                visibleListModel.addElement(new NavigationItem(stepDisplayName, currentState, false));
