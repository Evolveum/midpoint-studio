...
public class TraceParser {
    ...
    private static final Logger LOG = Logger.getInstance(TraceParser.class);
...
//			} else
            if (visitable instanceof PrismReferenceValue) {
                ...
                if (refVal.getObject() == null && refVal.getOid() != null && refVal.getOid().startsWith(SchemaConstants.TRACE_DICTIONARY_PREFIX)) {
                    ...
                    if (entry == null) {
                        LOG.error("No dictionary entry #" + id);
                    } else if (entry.getObject() == null) {
						LOG.error("No object in dictionary entry #" + id);
                    } else if (entry.getObject().asReferenceValue().getObject() == null) {
						LOG.error("No embedded object in dictionary entry #" + id);
                    } else {
    ...
    public TracingOutputType getObject(InputStream stream, boolean isZip) throws IOException, SchemaException {
        ...
        if (isZip) {
            try (ZipInputStream zis = new ZipInputStream(stream)) {
                ...
                if (zipEntry != null) {
                    ...
                    LOG.error("No zip entry in input file");        // TODO error handling
                    object = null;
                }
            }
        } else {
        ...
        if (object instanceof TracingOutputType) {
            ...
            LOG.error("Wrong object type in input file: " + object);
            return null;
        }
    }
